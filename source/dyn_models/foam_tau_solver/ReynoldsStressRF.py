...
class ReynoldsStressRF:
    ...
    def __init__(
        ...
        self.k, self.V1, self.V2, self.V3, self.C, self.NP = self._tau2PhysParams(
            self.tau)
    ...
    def getDeltaThetaVABC(self, tauOld, tauNew, *indexList):
        ...
        if np.shape(indexList)[0] == 0:
            ...
            ThetaVANew, ThetaVBNew, ThetaVCNew = self.getThetaVABC(
                tauNew, indexList)
    ...
    def getThetaVABC(self, tau, *indexList):
        ...
        for i in range(self.nMesh):
            ...
            if np.shape(indexList)[0] == 0:
                ...
                VMatrixOrg = np.array([V1[i], V2[i], V3[i]])
                # mutiply by -1 if switching vectors violate the right-hang
                # rule
                VMatrix = np.array([self.rotFactor[int(indexList[i])] *
                                    VMatrixOrg[indexDict[int(
                                        indexList[i])][0]],
                                    VMatrixOrg[indexDict[int(
                                        indexList[i])][1]],
                                    VMatrixOrg[indexDict[int(
    ...
    def evalVecNorm(self, VMOld, VMNew):
        ...
        deltaV = np.linalg.norm(
            rotM - np.diag(np.diag(rotM))) / np.linalg.norm(rotM)
    ...
    def plotEigenSystem(self, rotM):
        ...
        ax = Axes3D(fig)
        for iterN in range(3):
            ax.plot([0, rotM[iterN, 0]], [0, rotM[iterN, 1]],
                    [0, rotM[iterN, 2]], colorList[iterN])
        plt.show()
        return 0
    ...
    def adjustSign(self, rotM):
        ...
        if np.inner(rotM_adjust[:, 0], rotM[:, 2]) < np.inner(
                rotM[:, 0], rotM[:, 2]):
            return rotM_adjust
        else:
            return rotM
    ...
    def crossSolver(self, rotM, e23, e33):
        ...
        for iterN in range(2):
            rotM_C1 = np.cross(
                rotM[:, 1], [rotM[0, 2], e23[iterN], e33[iterN]])
    ...
    def setSign1(self, rotM):
        ...
        for iterN in range(len(sign02List)):
            ...
            if abs(np.inner(rotM[1], rotM[1]) -
                   1) < 1e-6 and abs(np.inner(rotM[2], rotM[2]) -
                                     1) < 1e-6:
                break
    ...
    def setSign2(self, rotM):
        ...
        for iterN in range(len(sign02List)):
            ...
            if np.sign(rotM_copy[0, 0]) == np.sign(rotM[0, 0]):
                det = abs(
                    np.linalg.det(
                        np.linalg.inv(rotM_copy) -
                        np.transpose(rotM_copy)))
                if det < detRef:
                    signIndex = iterN
                    detRef = det
    ...
    def reconstructTauFromQuatInBary(
            self,
            TauOld,
            deltaXbary,
            deltaYbary,
    ...
    def plotTauOnBaycentric(
        ...
        if tauComp == 'None':
            ...
            plt.plot([0, 1, 0.5, 0.5, 0], [
                     0, 0, 3**0.5 / 2.0, 3**0.5 / 2.0, 0], 'b-')
            plt.show()
        else:
            k, V1, V2, V3, C, NP = self._tau2PhysParams(tauPerturb)
            X = self._C2X(C)
            p3, = plt.plot(X[:, 0], X[:, 1], sampleLS,
                           markeredgecolor='grey', markerfacecolor='None', alpha=0.5)
            plt.plot(X[0, 0], X[0, 1], 'bo', markeredgecolor='b',
    ...
    def _tau2PhysParams(self, tauArray):
        ...
        for i in range(self.nMesh):
            ...
            if detV < 0:
                vContainer = np.dot(
                    np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]), vContainer)
                NPs[i, 0] = -1
        ...
        for i in range(self.nMesh):
            ...
            if len(deltaThetaV) > 0:
                ...
                if len(deltaThetaV[0]) == 1:
                    ...
                    V[indexDict[int(deltaThetaV[0][1][i])]
                      [0], :] = RMatrix[0, :]
                    V[indexDict[int(deltaThetaV[0][1][i])]
                      [1], :] = RMatrix[1, :]
                    V[indexDict[int(deltaThetaV[0][1][i])]
