...
class Inverse(object):
    ...
    def __init__(self, nsamples, t_interval, t_end, max_iteration,
        ...
        nsamples : int
            Ensemble size.
        t_interval : float
            Time interval between inversion steps
            (i.e. observations/data assimilation steps).
        t_end : float
            Final time.
        max_iteration : int
        ...
        dyn_model : DynModel
            Dynamic model.
        input_dict : dict[str]
            Inversion technique specific inputs.
        """
        self.name = 'Generic inversion technique'
        self.dyn_model = dyn_model

    def __str__(self):
        str_info = 'An empty inversion technique.'
        return str_info

    def solve(self):
        """ Implement the inversion technique. """
        pass
...
class Inverse2(Inverse):
    ...
    def __init__(self, nsamples, t_interval, t_end, max_iteration,
        ...
        Inputs in ``input_dict``:
            ...
            * **sensitivity_only** (``bool``, ``False``) -
              Perform initial sampling but no inversion.
            ...
            * **convergence_option** (``str``, ``variance``) -
              ...
              criteria,  ``residual`` to use the residual convergence
              criteria. See documentation for more information.
        ...
        """
        self.name = 'Generic inversion technique'
        self.short_name = 'generic'
        self.dyn_model = dyn_model
        self.nsamples = nsamples
        self.nstate = self.dyn_model.nstate
        self.nstate_obs = self.dyn_model.nstate_obs
        ...
        self.time_array = np.arange(0.0, self.t_end, self.t_interval)
        self.iteration_array = np.arange(0, self.max_iteration)
        # initialize states: these change at every iteration
        self.time = 0.0  # current time
        self.i_time = int(0)  # current inversion (obs/DA) step
        self.i_iteration = int(0)  # current forward step
        ...
        # TODO: how does it handle different times?
        self._misfit_norm = []
        self._sigma_hx_norm = []
    ...
    def __str__(self):
        str_info = self.name + \
            ...
            '\n   Observation interval: {}'.format(self.t_interval) + \
    ...
    def solve(self):
        ...
        This is the main method for the inversion technique.
        ...
        **Updates:**
        ...
        for time in self.time_array:
            self.time = time + self.t_interval
            self.i_time += 1
            if self._verb >= 1:
                print("\nData-assimilation step: {}".format(self.i_time) +
            ...
            for iteration in self.iteration_array:
                ...
                if self._verb >= 1:
                    print("\n  Forward step: {}".format(self.i_iteration))
                ...
                if conv and not self.reach_max_flag:
                    ...
                    self._store_misfits()
                    break
                if self.i_iteration == self.max_iteration:
                    self._store_misfits()
    ...
    def plot(self):
        ...
        if self._stationary_flag:
            iter = np.arange(self.i_iteration) + 1
        else:
            iter = np.arange(self.i_time) + 1
        fig, ax = plt.subplots()
    ...
    def save(self):
        ...
        for iter, value in enumerate(self.state_vec_analysis_all):
            np.savetxt(xadir + os.sep + 'Xa_{}'.format(iter+1), value)
        for iter, value in enumerate(self.state_vec_forecast_all):
            np.savetxt(xfdir + os.sep + 'Xf_{}'.format(iter+1), value)
        for iter, value in enumerate(self.model_obs_all):
            np.savetxt(hxfdir + os.sep + 'HXf_{}'.format(iter+1), value)
        for iter, value in enumerate(self.obs_all):
            np.savetxt(odir + os.sep + 'obs_{}'.format(iter+1), value)
        for iter, value in enumerate(self.obs_error_all):
            np.savetxt(oedir + os.sep + 'R_{}'.format(iter+1), value)
        ...
        if self._debug_flag:
            for key, value in debug_dict.items():
                fname = self._debug_dir + os.sep + key + \
                    '_{}_{}'.format(self.i_time, self.i_iteration)
    ...
    def _check_convergence(self):
        ...
        conv_variance = self._misfit_norm[self.i_iteration - 1] < \
            self._sigma_obs_norm[self.i_iteration - 1]
        residual = self._iteration_residual(
            self._misfit_norm, self.i_iteration-1)
    ...
    def _report(self):
        ...
        str_report += "    Norm of standard deviation of HX: {}".format(
            self._sigma_hx_norm[self.i_iteration - 1]) + \
...
class EnKF(Inverse2):
    ...
    def __init__(self, nsamples, t_interval, t_end, max_iteration,
        ...
        super(self.__class__, self).__init__(
            nsamples, t_interval, t_end, max_iteration, dyn_model,
            input_dict)
    ...
    def _correct_forecasts(self):
        ...
        **Updates:**
        ...
        for ipenalty in self.penalties:
            ...
            # if self.i_iteration > 1 and self.cost3_all[-1] > 0.1*self.cost2_all[-1]: # Function, not too large
            # lamda=self.lamda_all[-1] # Function, not too large

            # if self.i_iteration-1 < ipenalty['rampup']:
            #     ramp = float(self.i_iteration-1) / ipenalty['rampup']
            # else:
            #     ramp = 1.0
