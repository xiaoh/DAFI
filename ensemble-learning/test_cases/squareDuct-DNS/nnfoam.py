...
class Model(PhysicsModel):
    ...
    def __init__(self, inputs_dafi, inputs_model):
        ...
        # observations
        # read observations
        u1 = np.loadtxt(obs_file + 'postProcessing/sampleDict/0/line_x1_UDNS.xy')[:, 1:]
        u2 = np.loadtxt(obs_file + 'postProcessing/sampleDict/0/line_x2_UDNS.xy')[:, 1:]
        u3 = np.loadtxt(obs_file + 'postProcessing/sampleDict/0/line_x3_UDNS.xy')[:, 1:]
        u4 = np.loadtxt(obs_file + 'postProcessing/sampleDict/0/line_x4_UDNS.xy')[:, 1:]
        u1[:, 1:] = u1[:, 1:] * 50
        u2[:, 1:] = u2[:, 1:] * 50
        u3[:, 1:] = u3[:, 1:] * 50
        u4[:, 1:] = u4[:, 1:] * 50

        self.obs = np.concatenate([u1.flatten(), u2.flatten(), u3.flatten(), u4.flatten()])
    ...
    def state_to_observation(self, state_vec):
        ...
        for isample in range(self.nsamples):
            ...
            u2 = np.loadtxt(file + '/line_x2_U.xy')[:, 1:]
            u3 = np.loadtxt(file + '/line_x3_U.xy')[:, 1:]
            u4 = np.loadtxt(file + '/line_x4_U.xy')[:, 1:]
            u1[:, 1:] = u1[:, 1:] * 50
            u2[:, 1:] = u2[:, 1:] * 50
            u3[:, 1:] = u3[:, 1:] * 50
            u4[:, 1:] = u4[:, 1:] * 50
            state_in_obs[:, isample] = np.concatenate([u1.flatten(), u2.flatten(), u3.flatten(), u4.flatten()])
        return state_in_obs
...
def _run_foam(foam_dir, iteration, timeprecision):
    ...
    logfile = os.path.join(foam_dir, 'log.sample')
